...
**Last updated:** 2026-05-02 (56 tools, 97 server endpoints + SignalR hub, 1,911 tests across 10 projects, JS SDK covering the server, V001–V022 migrations, Phase 90 Public Release Readiness shipped)
...
| Frontend SDK | ✅ Implemented | `sdk/js/` — TypeScript `SovrantClient` covering the 97-endpoint server (incl. `updateTeamProfile` for team profiles + Command Center state endpoint), SSE parser, React `useChat()` hook, 75+ type definitions |
...
| SQLite persistence layer | ✅ Implemented | `IStorageProvider` + `SqliteStorageProvider` + 22 versioned migrations V001–V022. Stores: sessions (+ titles, entry provider), memory, audit, credentials, token usage, workspaces, projects, teams (+ run profile), missions, swarm events, coordination events, hooks, workspace_settings, MCP/LSP servers, user preferences, provider profiles. See [persistence.md](persistence.md). |
| Unit test suite | ✅ 1,911 passing | 10 projects: Runtime, Agents, Tools, Server, Api, Runtime.Documents, Commands, McpServer, Lsp, Integration |
