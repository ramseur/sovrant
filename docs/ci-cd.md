...
> **Phase 59 — Graduated Tool Tiers:** All 49+ tools are classified into four tiers (Safe, Moderate, Dangerous, Escalation) via `GraduatedToolTiers`. The `CiPermissionPolicy` allows Safe, Moderate, and Dangerous tools (Bash/PowerShell are expected in CI). Escalation-tier tools (Agent, Team, Swarm, Mission) are denied by default in CI — they spawn sub-processes or long-running orchestrations not suitable for headless CI runs. The Trust Boundary (Phase 58) is active in CI mode: PII/corporate data sanitization applies to all outbound LLM calls, and the ethical harness blocks harmful content generation.
