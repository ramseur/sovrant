...
| **v1.2** | Phase 99 | Private sessions and agent runs — per-record privacy toggle; masked rows in Command Center; server-side enforcement |
...
| User Dashboard — regular users get a personal dashboard scoped to their active workspace/project showing their own sessions, agent runs, and recent activity; Command Center remains unchanged as the admin-only aggregated view across all workspaces, users, and agents; User Dashboard is accessible from the rail nav for all users while Command Center remains admin-only; role gate enforced server-side; Web + Desktop parity | Phase 98 | High |
| Private sessions and agent runs — users can mark any session or agent run as private via a toggle in the chat header and Agents UI; private records still appear in the Command Center and User Dashboard as masked rows (showing only that a private session/run exists, not its title, prompt, or content) so activity counts remain accurate; admins see the masked placeholder and cannot drill into private content; only the owning user can see the full title, prompt, content, and run history of their private records; server-side enforcement ensures private content is never returned in aggregation or detail queries for any other user regardless of role; audit log records the privacy toggle event | Phase 99 | High |
