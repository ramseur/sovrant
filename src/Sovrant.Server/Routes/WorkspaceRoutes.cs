...
internal static class WorkspaceRoutes
{
        ...
        CreateWorkspaceRequest req, HttpContext ctx, IWorkspaceService svc, CancellationToken ct)
    {
        ...
        if (!ctx.IsAdmin())
            return Results.Json(new { error = "Only global admins can create team workspaces." }, statusCode: StatusCodes.Status403Forbidden);
        ...
        string id, string? layer, HttpContext ctx, IWorkspaceService svc, CancellationToken ct)
    {
        ...
        var memory = await svc.ListMemoryAsync(id, layer, viewerUserId: viewerUserId, ct: ct).ConfigureAwait(false);
